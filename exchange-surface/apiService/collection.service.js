...
import { cookieConfig } from '../middleware/token';
...
export const fetchCollectionByName = async (req, res, collectionName) => {
  ...
  try {
    ...
    // VAPT Cookie Task
    res.cookie('JSESSIONID', jsessionid, { maxAge: 1800000, ...cookieConfig });
...
export const refreshAndFetchUserData = async (req, res, user) => {
  try {
    const token = await getCookieToken(req);
    console.log('refreshAndFetchUserData');
    const reqHeaders = {
      headers: {
        JSESSIONID: token,
        authorization: user?.token || '',
    ...
    const url = `${getBackendServerUrl(req)}${AUTH_ENDPOINT}/refresh-user/${user.uuid}`;
    const response = await axios.get(url, reqHeaders);
    const { data, headers } = response;
    const collectionItemData = data;
    const { jsessionid } = headers;
