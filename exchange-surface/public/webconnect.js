window.addEventListener('DOMContentLoaded', async function () {
  ...
  if (collectionId && collectionItemId && itemData) {
    ...
    const innerDataGroup = window.document.querySelectorAll('[data-ref-field]');
  ...
  loadSessionSubTenantDataIntoElements();
  ...
  if (bslElems) {
    (async () => {
      ...
      for (const element of bslElems) {
        ...
        let bslBrowserStorageData = await getBSLData(bslKey);
        let bsltBrowserStorageData = await getBSLData(bslTextKey);
...
const loadSessionDataIntoElements = (
  isModal = false,
  sessionKey = '',
  query = '',
  element = '',
  ...
  let sessionAttributes = element.querySelectorAll(`${SESSION_KEY}`);

  if (isModal) {
    const QUERY = query ? query : '[id^=modal-container]';
    sessionAttributes = element.querySelectorAll(`${QUERY} ${SESSION_KEY}`);
  }
...
const loadSessionTenantDataIntoElements = (
  isModal = false,
  sessionKey = '',
  query = '',
  element = '',
...
const loadSessionUserSettingsDataIntoElements = (
  isModal = false,
  sessionKey = '',
  query = '',
  element = '',
  ...
  let sessionAttrElems = element.querySelectorAll(`${SESSION_KEY}`);
  if (isModal) {
    const QUERY = query ? query : '[id^=modal-container]';
    sessionAttributes = element.querySelectorAll(`${QUERY} ${SESSION_KEY}`);
  }
...
const loadSessionSubTenantDataIntoElements = (
  isModal = false,
  sessionKey = '',
  query = '',
  element = '',
  ...
  element = element || document;
  const SESSION_KEY = sessionKey ? sessionKey : '[data-session-sub-tenant]';
  const SESSION_FIELD_KEY = 'data-session-sub-tenant';
  const SESSION_ITEM_FROM_KEY = 'sessionSubTenant';
  let sessionAttrElems = element.querySelectorAll(`${SESSION_KEY}`);
  if (isModal) {
    const QUERY = query ? query : '[id^=modal-container]';
    sessionAttributes = element.querySelectorAll(`${QUERY} ${SESSION_KEY}`);
  ...
  if (sessionAttrElems) {
    ...
    if (loggedInSubTenantData && loggedInSubTenantData !== 'undefined') {
      let loggedInSubTenant = parseLSJSONStrToJSON('subTenant');
      sessionAttrElems.forEach((element) => {
        loadDataIntoElements(element, SESSION_FIELD_KEY, SESSION_ITEM_FROM_KEY, loggedInSubTenant);
      });
    }
  }
};
...
const socketIoInitialization = async () => {
  const socketPlugin = await isSocketIOPluginInstalled();
  if (!socketPlugin) {
    console.log('Socket IO plugin is not installed.');
    return;
  }
  const currentUser = fetchLoggedInUserJson();
  if (!currentUser) {
    console.log('No logged in user found for socket connection.');
    return;
  }
  ...
  if (currentTenant?.uuid) {
    authPayload.tenantId = currentTenant.uuid;
  }
  ...
  socket.on('connect', () => {
    console.log(
      '==> Socket connection established for user:',
      currentUser.uuid,
      'socketId:',
      socket.id,
    );
    socket.on('socket-message', (data) => {
      const { notificationType, socketIoContent } = data;
      switch (notificationType) {
        case 'toastSuccess':
          toastr.success(socketIoContent);
          break;
        case 'toastError':
          toastr.error(socketIoContent);
          break;
        case 'toastInfo':
          toastr.info(socketIoContent);
          break;
        case 'toastWarning':
          toastr.warning(socketIoContent);
          break;
        case 'alert':
          alert(socketIoContent);
          break;
      }
    });
  });
  socket.on('collection-data-add', (data) => {
    const { collectionName, socketData } = data;
    const socketAlertComponents = document.querySelectorAll('[data-socket-alert-component]');
    socketAlertComponents.forEach((component) => {
      const componentCollection = elementAttribute(component, 'data-socket-alert-collection');
      const triggerAction = elementAttribute(component, 'data-socket-alert-trigger');
      if (componentCollection === collectionName && triggerAction === 'add') {
        component.style.display = 'block';
        let existingDataAttr = component.getAttribute('data-socket-alert-data');
        let newDataArray = [];
        try {
          if (existingDataAttr) {
            const parsed = JSON.parse(existingDataAttr);
            if (Array.isArray(parsed)) {
              newDataArray = [...parsed, socketData];
            } else {
              newDataArray = [parsed, socketData];
            }
          } else {
            newDataArray = [socketData];
          }
        } catch (err) {
          newDataArray = [socketData];
        }
        component.setAttribute('data-socket-alert-data', JSON.stringify(newDataArray));
      }
    });
  });
  socket.on('collection-data-update', (data) => {
    // console.log('data update from socket', data);
    //Handle update here if needed
  });
  socket.on('collection-data-delete', (data) => {
    const { collectionName, socketData } = data;
    const socketAlertComponents = document.querySelectorAll('[data-socket-alert-component]');
    socketAlertComponents.forEach((component) => {
      const componentCollection = elementAttribute(component, 'data-socket-alert-collection');
      const triggerAction = elementAttribute(component, 'data-socket-alert-trigger');
      if (componentCollection === collectionName && triggerAction === 'delete') {
        component.style.display = 'block';
        component.setAttribute('data-socket-alert-data', JSON.stringify(socketData));
        component.removeAttribute('data-socket-alert-target-append');
      }
    });
  });
