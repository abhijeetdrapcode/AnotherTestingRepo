...
export const executeExternalApiAndProcess = async (
  ...
  console.log('🚀 ~ executeExternalApiAndProcess ~ setting:', setting, '~ hasHeaders:', hasHeaders);
  ...
  console.log('🚀 ~ executeExternalApiAndProcess ~ result:', result);
  ...
  responseData = spreadResponse
    ? { ...responseData, externalApiMiddlewareId, responseSavedCollection, responseSavedItemsUuid }
    : responseData;
      ...
      needleList
        ...
        ?.forEach((prop) => {
          ...
          if (dtoExternalApiType === MYSQL) {
            switch (fieldSchema.type) {
              case 'text':
                dataOfItem = dataOfItem ? `'${dataOfItem.toString()}'` : dataOfItem;
                break;
              case 'boolean':
