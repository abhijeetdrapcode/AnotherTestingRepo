...
export const checkPermissionLevelSecurity = async (
  ...
  try {
    let currentUser = await authenticateUser(db, projectId, authorization);
    if (!currentUser) {
      currentUser = {};
    }
    ...
    const currentUserSetting = await extractUserSettingFromUserAndTenant(
      db,
      projectId,
      currentUser,
      currentTenant,
    ...
    const processItem = (item) => {
      for (let i = 0; i < permissionLevelSecurity.length; i++) {
        const { fieldName, permission, allowedPermissions, allowedReferenceFields } =
          permissionLevelSecurity[i];
        if (!Object.prototype.hasOwnProperty.call(item, fieldName)) continue;

        if (permission === 'restricted') {
          const allowedPermissionsExists =
            Array.isArray(allowedPermissions) && allowedPermissions.length > 0;
          const hasAllowedReferenceFields =
            Array.isArray(allowedReferenceFields) && allowedReferenceFields.length > 0;
          const userHasPermission =
            allowedPermissionsExists &&
            allowedPermissions.some((perm) => effectivePermissions.includes(perm));

          if (allowedPermissionsExists) {
            if (userHasPermission && hasAllowedReferenceFields && Array.isArray(item[fieldName])) {
              item[fieldName] = item[fieldName].map((refField) =>
                allowedReferenceFields.reduce((acc, field) => {
                  if (refField[field] !== undefined) acc[field] = refField[field];
                  return acc;
                }, {}),
              );
            } else {
          ...
          } else if (hasAllowedReferenceFields && Array.isArray(item[fieldName])) {
            item[fieldName] = item[fieldName].map((refField) =>
              allowedReferenceFields.reduce((acc, field) => {
                if (refField[field] !== undefined) acc[field] = refField[field];
                return acc;
...
export const removeEmptyValues = (obj) => {
  const skipEmptyValues = obj?.skipEmptyValues;

  if (obj?.selectedItemsIds) delete obj?.selectedItemsIds;
  if (skipEmptyValues) delete obj?.skipEmptyValues;

  if (!skipEmptyValues) return obj;
...
export const convertToFindOneAndUpdateQuery = (query) => {
  const updatePipeline = [];
  if (query.$set) {
    const setFields = { ...query.$set };
    // Handle version conversion and increment
    if (query.$set.version === 'INC') {
      setFields.version = { $add: [{ $toInt: { $ifNull: ['$version', 0] } }, 1] };
    }
    // Wrap any string starting with "$" in $literal
    for (const [key, value] of Object.entries(setFields)) {
      if (typeof value === 'string' && value.startsWith('$')) {
        setFields[key] = { $literal: value };
      }
    }
    updatePipeline.push({ $set: setFields });
  }

  if (query.$unset) {
    const unsetFields = Object.keys(query.$unset);
    if (unsetFields.length > 0) updatePipeline.push({ $unset: unsetFields });
  }
