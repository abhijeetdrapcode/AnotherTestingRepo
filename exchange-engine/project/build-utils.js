...
export const renderHeadSection = (mainPath, context, pluginsWithCssAndJs, page, data) => {
  // console.log('The code is reaching here : ', context);
  const { plugins, project, publishTimestamp, customCSSURL, environment } = context;
  console.log('This is the loadingIconKey from Build utils: ', context.project.loadingIconKey);
  ...
  allFormWithValidationAttrs.forEach((form) => {
    ...
    if (validation) {
      ...
      validationRules.forEach((valRul) => {
        ...
        if (valRul.fieldType === 'time_slot') hiddenFieldNames.push(valRul.field);
      });
      ...
      const validationDetail = { ignore, rules, messages };
